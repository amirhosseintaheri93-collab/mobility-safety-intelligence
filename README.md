...
- Includes optional, restrained interface feedback sounds across the scenario game and research app. Sounds are off by default; the application has no background music.
...
The app's three short interface sounds are generated locally by
`scripts/generate_audio_assets.py`. They are original synthetic effects—not sampled
recordings and not reproductions of an existing composition or performer. There is
no background music. Regenerate the effects with:
